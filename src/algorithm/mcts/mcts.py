...
class MCTS: 
  ...
  def search(self, node): 
    ...
    log.info(f"MCTS suggests: {mcts_action}, visits: {best_child.visits}, win_rate: {win_rate:.2f}")
    ...
    if dist_to_enemy <= 2:
        log.warning(f"DANGER! MCTS chose a suicide move {mcts_action} (Dist {dist_to_enemy}). Overriding...")
        ...
        if best_safe_action:
            log.info(f"Safety Override: Switched to {best_safe_action} (Dist {max_dist})")
            return best_safe_action
        else:
            log.warning("No safe moves available. Accepting fate.")
