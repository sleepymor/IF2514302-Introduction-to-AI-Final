...
class ResultsExporter:
    ...
    def export_to_excel(self, results: List[GameResult], filename: str = None):
        """
        Export results to Excel file with progress bar.
    ...
    def _create_summary_sheet(self, ws, analyzer: ResultsAnalyzer):
        ...
        data_iterator = tqdm(sorted(stats.items()), desc="  Writing Summary", leave=False) if HAS_TQDM else sorted(stats.items())
    ...
    def _create_raw_data_sheet(self, ws, analyzer: ResultsAnalyzer):
        ...
        # Write data with progress
        data_iterator = tqdm(raw_data, desc="  Writing Raw Data", leave=False) if HAS_TQDM else raw_data
    ...
    def _create_per_seed_sheet(self, ws, analyzer: ResultsAnalyzer):
        ...
        # Write data with progress
        data_iterator = tqdm(sorted(per_seed_stats.items()), desc="  Writing Per-Seed Stats", leave=False) if HAS_TQDM else sorted(per_seed_stats.items())
    ...
    def export_to_csv(self, results: List[GameResult], filename: str = None):
        ...
        with open(summary_filepath, 'w', newline='', encoding='utf-8') as f:
            if stats:
                writer = csv.DictWriter(f, fieldnames=stats[list(stats.keys())[0]].keys())
                writer.writeheader()
