...
Calculates statistics and metrics from test results.
"""
...
class ResultsAnalyzer:
    """Analyzes benchmark test results and computes statistics."""

    def __init__(self, results: List[GameResult]):
        """Initialize analyzer.

        Args:
            results: List of GameResult objects from benchmark tests.
        """
    ...
    def get_stats_by_algorithm(self) -> Dict[str, Dict]:
        ...
        Returns:
            Dictionary with comprehensive stats for each algorithm.
        """
        stats_by_algo = defaultdict(
            lambda: {
                "total_tests": 0,
                "wins": 0,
                "traps": 0,
        ...
        # Mapping from result label to stats key
        result_to_key = {
            "win": "wins",
            "trap": "traps",
            "caught": "caught",
        ...
        for algo, stats in stats_by_algo.items():
            if stats["total_tests"] > 0:
                ...
                final_stats[algo] = {
                    ...
                    "Total Tests": total,
                    "Wins": stats["wins"],
                    "Win Rate (%)": round(100 * stats["wins"] / total, 2),
                    "Deaths by Trap": stats["traps"],
                    "Death by Trap Rate (%)": round(100 * stats["traps"] / total, 2),
                    "Deaths by Enemy": stats["caught"],
                    "Death by Enemy Rate (%)": round(100 * stats["caught"] / total, 2),
                    "Timeouts": stats["timeout"],
                    "Timeout Rate (%)": round(100 * stats["timeout"] / total, 2),
                    "Errors": stats["errors"],
                    "Average Moves to Win": round(stats["total_moves"] / total, 2),
                    "Average Turns": round(stats["total_turns"] / total, 2),
                }

        return final_stats

    def get_stats_by_seed(self, algorithm: str = None) -> Dict[int, Dict]:
        """Get statistics breakdown by seed.

        Args:
            algorithm: If specified, filter stats to this algorithm only.

        Returns:
            Dictionary with stats for each seed.
        """
        stats_by_seed = defaultdict(
            lambda: {
                "wins": 0,
                "traps": 0,
                "caught": 0,
        ...
        for seed, stats in sorted(stats_by_seed.items()):
            if stats["total"] > 0:
                ...
                final_stats[seed] = {
                    ...
                    "Deaths by Enemy": stats["caught"],
                    "Timeouts": stats["timeout"],
                    "Avg Moves": round(stats["total_moves"] / total, 2),
                }
